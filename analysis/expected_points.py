...
is intractable here due to 748 one-time items creating 2^748 possible states),
...
KEY OPTIMISATION -- TWO-STAGE SAMPLING:
  ...
  This avoids building a single 753-item probability distribution every roll.
...
KEY OPTIMISATION -- GPU ACCELERATION (CuPy):
  When an NVIDIA GPU is available and CuPy is installed, all large simulation
  arrays live on the GPU. Operations run as CUDA kernels, processing all
  simulations in parallel across thousands of GPU cores. If no compatible GPU
  is found, the script falls back to NumPy (CPU) automatically.

  CuPy requires an NVIDIA GPU with CUDA support. AMD GPUs (ROCm) and Intel
  GPUs (oneAPI) are not compatible. When running directly, the script detects
  the GPU and prompts the user to choose. When imported by other scripts, the
  backend is controlled via the use_gpu parameter on simulate().
...
# function: xp = cupy when GPU is selected, xp = numpy otherwise. This is a
...
# initialisation -- it only checks if the package is installed. The actual
# GPU check happens inside detect_backend() when called.
try:
    import cupy as cp
    _CUPY_INSTALLED = True
except ImportError:
    _CUPY_INSTALLED = False
    cp = None  # placeholder so "cp" is defined for type checks in to_cpu()


def detect_backend():
    """
    Checks whether a compatible NVIDIA GPU is available for CuPy acceleration.
    Returns a dictionary with the detection results.

    This function is safe to call multiple times -- it does not print anything
    or prompt the user. Printing and prompting are handled separately.

    Returns:
        dict with keys:
            "gpu_available": bool -- True if CuPy + NVIDIA GPU are working
            "gpu_name":      str or None -- GPU device name (e.g. "NVIDIA GeForce RTX 4070")
            "gpu_vram_gb":   float or None -- total VRAM in GB
    ...
    try:
        if cp.cuda.runtime.getDeviceCount() == 0:
            return {
                ...
                "gpu_name": None,
                "gpu_vram_gb": None,
                "reason": "CuPy is installed but no NVIDIA GPU was detected."
            }
        ...
        return {
            ...
            "reason": "NVIDIA GPU detected and CuPy is functional."
        }
        ...
        return {
            ...
            "gpu_name": None,
            "gpu_vram_gb": None,
            "reason": f"CuPy is installed but GPU initialisation failed: {e}"
        }


def prompt_backend():
    """
    Detects GPU availability and selects the backend automatically.

    If no GPU is available, prints a message and returns False (use CPU).
    If a GPU is available, shows the device info and returns True (use GPU).

    Returns:
        bool -- True to use GPU, False to use CPU.
    """
    info = detect_backend()

    if not info["gpu_available"]:
        print(f"GPU acceleration: UNAVAILABLE")
        print(f"  {info['reason']}")
        print(f"  Using CPU (NumPy) backend.\n")
        return False

    print(f"GPU detected: {info['gpu_name']} ({info['gpu_vram_gb']:.1f} GB VRAM)")
    print(f"  Using GPU (CuPy) backend.\n")
    return True
...
# functions work regardless of which backend is active by checking the array
# type directly, rather than relying on a global flag.
...
def to_cpu(x):
    ...
    """
    # Check for CuPy arrays by looking for the .get() method, which is
    # CuPy-specific. This avoids depending on the global cp variable being
    # defined (it won't be if CuPy isn't installed).
...
def simulate(config, max_rolls, simulations=100_000, seed=42, dist_checkpoints=None,
    ...
    When use_gpu is True (or auto-detected), all large arrays live on the GPU.
    The per-roll loop runs on the CPU but dispatches GPU kernels for all the
    heavy math. Small output arrays (one element per roll) stay on the CPU.
    ...
    Parameters:
        ...
        use_gpu:          controls the compute backend:
                            None  = auto-detect (use GPU if available, else CPU)
                            True  = force GPU (raises error if unavailable)
                            False = force CPU (even if GPU is available)
    ...
    #
    # The use_gpu parameter controls this:
    #   None  -> auto-detect: use GPU if available, else CPU (no prompt)
    #   True  -> force GPU (error if unavailable)
    #   False -> force CPU regardless of GPU availability
    if use_gpu is None:
        # Auto-detect silently (for imports and non-interactive use).
        info = detect_backend()
        _use_gpu = info["gpu_available"]
    elif use_gpu is True:
        info = detect_backend()
        if not info["gpu_available"]:
            raise RuntimeError(
                f"use_gpu=True but no compatible GPU found. {info['reason']}"
            )
        _use_gpu = True
    else:
        _use_gpu = False
...
# --- SAVE / LOAD RESULTS ------------------------------------------------------
#
# These functions decouple simulation from visualisation. Running the simulation
# once produces a CSV (summary arrays) and an NPZ (raw distribution snapshots).
# Plotting scripts load these files instead of re-importing and re-running the
# simulation.
#
# CSV holds one row per roll with columns:
#   roll, mean, std, marginal, s1_prob, s2_prob, s3_ot_remaining,
#   p1, p5, p10, p25, p50, p75, p90, p95, p99
#
# NPZ holds the dist_snapshots: 100k cumulative point values at each checkpoint
# roll, keyed as "roll_10", "roll_100", etc. Also stores n_s3_ot_total as a
# scalar for the S3 drain plot.

def save_results(results, output_dir):
    """
    Saves simulation results to CSV and NPZ files.
    ...
    Creates:
        output_dir/simulation_results.csv  -- summary arrays (one row per roll)
        output_dir/simulation_snapshots.npz -- raw 100k-value distributions
    """
    os.makedirs(output_dir, exist_ok=True)

    # -- CSV: summary arrays ---------------------------------------------------
    rolls = results["roll_numbers"]
    cum_mean = results["cumulative_mean"]
    cum_std = results["cumulative_std"]
    marginal = results["marginal_mean"]
    s1_prob = results["s1_transform_prob"]
    s2_prob = results["s2_complete_prob"]
    s3_rem = results["s3_ot_remaining_mean"]
    pct_vals = results["percentile_values"]   # list of lists, one per roll
    pct_labels = results["percentiles"]       # [1, 5, 10, 25, 50, 75, 90, 95, 99]
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        # Header row
        pct_columns = [f"p{p}" for p in pct_labels]
        writer.writerow(["roll", "mean", "std", "marginal", "s1_prob", "s2_prob",
                         "s3_ot_remaining", "n_s3_ot_total"] + pct_columns)

        # Data rows
        n_ot = results["n_s3_ot_total"]
        for i, roll_num in enumerate(rolls):
            row = [
                roll_num,
                f"{cum_mean[i]:.4f}",
                f"{cum_std[i]:.4f}",
                ...
                f"{s3_rem[i]:.2f}",
                n_ot,
            ] + [f"{v:.2f}" for v in pct_vals[i]]
            writer.writerow(row)

    print(f"Saved: {csv_path}")

    # -- NPZ: raw distribution snapshots ---------------------------------------
    snapshots = results.get("dist_snapshots", {})
    if snapshots:
        npz_data = {"n_s3_ot_total": np.array([results["n_s3_ot_total"]])}
        for roll_num, pts_array in snapshots.items():
            npz_data[f"roll_{roll_num}"] = np.array(pts_array)

        npz_path = os.path.join(output_dir, "simulation_snapshots.npz")
...
def load_results(output_dir):
    """
    Loads simulation results from CSV and NPZ files.

    Parameters:
        output_dir: directory containing simulation_results.csv and
                    simulation_snapshots.npz

    Returns:
        dictionary with the same structure as simulate()'s return value,
        so all downstream code (plots, analysis) works unchanged.
    """
    csv_path = os.path.join(output_dir, "simulation_results.csv")
    npz_path = os.path.join(output_dir, "simulation_snapshots.npz")
    ...
    with open(csv_path, "r") as f:
        ...
        rolls, means, stds, marginals = [], [], [], []
        s1_probs, s2_probs, s3_rems = [], [], []
        pct_values = []
        n_ot = None

        # Find percentile column indices (everything after "n_s3_ot_total")
        pct_start = header.index("n_s3_ot_total") + 1
        pct_labels = [int(h[1:]) for h in header[pct_start:]]  # "p5" -> 5

        for row in reader:
            rolls.append(int(row[0]))
            means.append(float(row[1]))
            stds.append(float(row[2]))
            marginals.append(float(row[3]))
            s1_probs.append(float(row[4]))
            s2_probs.append(float(row[5]))
            s3_rems.append(float(row[6]))
            n_ot = int(row[7])
            pct_values.append([float(v) for v in row[pct_start:]])

    results = {
        "roll_numbers": rolls,
        "cumulative_mean": means,
        "cumulative_std": stds,
        "marginal_mean": marginals,
        "s1_transform_prob": s1_probs,
        "s2_complete_prob": s2_probs,
        "s3_ot_remaining_mean": s3_rems,
        "n_s3_ot_total": n_ot,
        "percentiles": pct_labels,
        "percentile_values": pct_values,
        "dist_snapshots": {},
    }

    # -- Load NPZ (if it exists) -----------------------------------------------
    if os.path.exists(npz_path):
        with np.load(npz_path) as data:
            for key in data.files:
                if key.startswith("roll_"):
                    roll_num = int(key[5:])  # "roll_100" -> 100
                    results["dist_snapshots"][roll_num] = data[key]
...
if __name__ == "__main__":
    ...
    # Step 1: Detect GPU and prompt user for backend choice.
    # This happens immediately, before any slow operations.
    use_gpu = prompt_backend()

    # Step 2: Parse the CSV into a structured dictionary.
    config = parse_data(DATA_PATH)

    print("Running simulation...\n")

    # Step 3: Run simulations with the chosen backend.
    results = simulate(
        config,
    ...
    # Step 4: Save results to disk for use by plots.py and other scripts.
    save_results(results, RESULTS_DIR)

    # Step 5: Print the three summary tables and distribution analysis.
